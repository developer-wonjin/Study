...
bool solve_sudoku(int currIdx) {
    // currIdx위치의 공란에 중복없는 숫자값이 채워졌음을 보장받고 함수로 진입함
    
    // 마지막 칸 유효한 숫자로 채워졌으므로 true 리턴
    if (currIdx + 1 == emptyV.size()) return true;

    // nextIdx위치
    int nextIdx = currIdx + 1;
    

    int next_row = emptyV[nextIdx].first;
    int next_col = emptyV[nextIdx].second;

    for (int num = 1; num <= 9; num++) {
        if (duplicate(next_row, next_col, num)) continue;

        board[next_row][next_col] = num;
        if (solve_sudoku(nextIdx)) return true;  // 해결되었으면 종료
        board[next_row][next_col] = 0;  // 백트래킹
    }
    return false;
}
