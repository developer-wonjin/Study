...
-- ************************************************
	...
	-- T1, T2 조인
	SELECT  T1.CUS_ID ,
			T2.ITM_ID , T2.EVL_LST_NO, T2.CUS_ID
	FROM    M_CUS T1
			,T_ITM_EVL T2 
	WHERE   T1.CUS_ID = T2.CUS_ID
	...
	--- T1, T2, T3 조인만하고 집계는 안했을 때
	SELECT  T1.CUS_ID ,
			T2.ITM_ID , T2.EVL_LST_NO, T2.CUS_ID, 
			T3.ORD_SEQ, T3.CUS_ID
	FROM    M_CUS T1
			,T_ITM_EVL T2
			,T_ORD T3
	WHERE   T1.CUS_ID = T2.CUS_ID
...
-- ************************************************
	...
	SELECT  T1.CUS_ID ,SUM(T1.EVL_CNT) EVL_CNT ,SUM(T1.ORD_CNT) ORD_CNT
	FROM    (
			SELECT  T1.CUS_ID ,COUNT(*) EVL_CNT ,NULL     ORD_CNT
			FROM    M_CUS T1
					,T_ITM_EVL T2
			WHERE   T1.CUS_ID = T2.CUS_ID
			...
			GROUP BY T1.CUS_ID
			UNION ALL
			SELECT  T1.CUS_ID ,NULL     EVL_CNT ,COUNT(*) ORD_CNT
			FROM    M_CUS T1
					,T_ORD T3
